const stats = [
  ...
  { num: "CHF 0", label: "To start — free 90-min diagnostic session" },
];

export default function Stats() {
  return (
    <div className="bg-navy-deep border-t border-navy-mid py-9 px-16">
      <div className="max-w-7xl mx-auto grid grid-cols-4 gap-6">
        {stats.map((s) => (
          <div key={s.num} className="text-center">
            <div className="font-serif text-4xl text-teal">{s.num}</div>
            <div className="text-xs text-teal-muted mt-1.5 leading-snug">{s.label}</div>
          </div>
        ))}
      </div>
    </div>
